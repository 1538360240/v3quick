...
pool_alloc(struct zproto *z, size_t sz)
{
        ...
        if (c->last < sz) {
				...
				need -= sizeof(*n);
				n = (struct pool_chunk *)malloc(need + sizeof(*n));
				n->next = NULL;
				n->start = sz;
				n->last = need - sz;
				c->next = n;
...
record(struct zproto *z, struct zproto_record *proto)
{
        int err;
        char name[64];
		struct zproto_record *n = (struct zproto_record *)pool_alloc(z, sizeof(*n));
		memset(n, 0, sizeof(*n));
		n->next = proto->child;
		n->parent = proto;
...
buffer_check(struct zproto_buffer *zb, int sz, int pack)
{
        if (zb->cap < (sz + zb->start)) {
                ...
                zb->p = (uint8_t *)realloc(zb->p, zb->cap);
                assert(zb->cap >= sz);
        }
        ...
        if (zb->pack_cap < pack) {
                ...
                zb->pack_p = (uint8_t *)realloc(zb->pack_p, zb->pack_cap);
